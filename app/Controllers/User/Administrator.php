...
use App\Models\Sidesa\User_administrator_model;
...
class Administrator extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->Administrator_model = new User_administrator_model();
        helper('zakezone');
    }
    ...
    public function dashboard()
    {
        ...
        $data = [
            ...
            'page_title' => view('sidesa/layout/user/content-page-title', ['title' => 'Kalender Perencanaan (PROTOTYPE)', 'li_1' => 'Administrator', 'li_2' => 'Dashboard']),
        ];
        return view('sidesa/user/administrator/apps-calendar', $data);
    }
    ...
    public function role_management()
    {
        ...
        $notif->set('read', "Y");
        $notif->where('target', "2");
        $notif->where('jenis_file', "Role Akses");
        $notif->orWhere('target', "2");
        ...
        if ($cari) {
            $get_role = $this->Administrator_model->getRole($cari);
        } else {
            $get_role = $this->Administrator_model->defaultgetRole();
        }
        ...
        $data = [
            ...
            'page_title' => view('sidesa/layout/user/content-page-title', ['title' => 'Role Management', 'li_1' => 'Administrator', 'li_2' => 'Role Management']),
            'start' => $start,
        ...
        if ($cari) {
            $data['total_rows'] = $this->Administrator_model->getRole($cari)->countAllResults();
        } else {
            $data['total_rows'] = $this->Administrator_model->defaultgetRole()->countAllResults();
        }

        // dd($data['getrole']);

        return view('sidesa/user/administrator/apps-role', $data);
    }
    ...
    public function role_edit($user_id, $kd_wilayah)
    {
        ...
        $data = [
            'title' => 'Role',
            'page_title' => view('sidesa/layout/user/content-page-title', ['title' => 'Role Edit', 'li_1' => 'Administrator', 'li_2' => 'Role Management']),
            'user' => $this->db->table('sidesa_user')->getWhere(['email' => $this->session->get('email_sidesa')])->getRowArray(),
            ...
            'roleedit' => $this->db->table('wilayah_33')->select(['id_desa', 'nm_kab', 'nm_kec', 'nm_desa'])->where('id_desa >', 2)->orderBy('id_prov ASC')->get()->getResultArray(),
        ...
        if ($this->validation->withRequest($this->request)->run()) {
            ...
            $this->Administrator_model->Notifikasiakses($input, $data['user'], $data['edit']);

            $this->session->setFlashdata('message', $flash);
            return redirect()->to('user/administrator/role_management');
        } else {
            return view('sidesa/user/administrator/apps-role-edit', $data);
        }
    }

    public function role_accessadm($role_id)
    {
        $this->session->remove('keyword');
        ...
        $data = [
            'title' => 'Role',
            'page_title' => view('sidesa/layout/user/content-page-title', ['title' => 'Konfigurasi Akses Menu', 'li_1' => 'Administrator', 'li_2' => 'Role Management']),
            'user' => $this->db->table('sidesa_user')->getWhere(['email' => $this->session->get('email_sidesa')])->getRowArray(),
            'role' => $this->db->table('sidesa_role')->getWhere(['id' => $role_id])->getRowArray(),
            'menu' => $tabmenu->get()->getResultArray(),
        ];
        return view('sidesa/user/administrator/apps-role-access', $data);
    }

    public function changeAccessadm()
    {
        $this->session->remove('keyword');
        $menu_idad = $this->request->getVar('menuIdad');
        $role_idad = $this->request->getVar('roleIdad');
        $data = [
            'role_id' => $role_idad,
            'menu_id' => $menu_idad
        ];
    ...
    public function profile()
    {
        ...
        $data = [
            'title' => 'Profile',
            'page_title' => view('sidesa/layout/user/content-page-title', ['title' => 'Profile', 'li_1' => 'Administrator', 'li_2' => 'Profile']),
            'user' => $this->db->table('sidesa_user')->getWhere(['email' => $this->session->get('email_sidesa')])->getRowArray(),
            'kab' => $this->Administrator_model->getMyprofile($sessionEmail),
        ];

        return view('sidesa/user/administrator/apps-profile', $data);
    }
    ...
    public function editprofile()
    {
        ...
        $data = [
            'title' => 'Edit Profile',
            'page_title' => view('sidesa/layout/user/content-page-title', ['title' => 'Edit Profile', 'li_1' => 'Administrator', 'li_2' => 'Edit Profile']),
            'user' => $this->db->table('sidesa_user')->getWhere(['email' => $this->session->get('email_sidesa')])->getRowArray(),
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!$this->validation->withRequest($this->request)->run()) {
                return redirect()->to('user/administrator/editprofile')->withInput();
            } else {
                $file = $this->request->getFile('image');
                $input = $this->request->getVar();
                $user_id = $this->session->get('id_sidesa');
                $this->Administrator_model->editProfile($user_id, $input, $file);
                ...
                return redirect()->to('user/administrator/profile');
            }
        }
        return view('sidesa/user/administrator/apps-editprofile', $data);
    }
    ...
    public function hapusUser($user_id, $nip)
    {
        ...
        $user = $this->db->table('sidesa_user')->getWhere(['email' => $this->session->get('email_sidesa')])->getRowArray();
        $this->Administrator_model->Notifikasihapus($nip, $user);
